...
像美术指导一样写词，不当约束收集器。核心方法：**先说清每张图的作用，再用一段话描述要改成什么，最后用短约束收口**。不要把同一组保护规则反复写三遍。
...
1. 参考图角色：先用 `image1 / image2 / image3...` 说明每张图的作用。
2. 主改图指令：一段话写清目标图要保留什么、替换什么、提升什么；不要逐图重复解释。
...
每张图必须点名分工，禁止“参考上传图片 / 原图 / 新图 / 参考图”这种模糊说法。多图时必须按上传顺序使用 `image1 / image2 / image3...` 编号，先说明每张图“是哪张图 + 本次生成负责什么”，再写提示词。
...
1. 第一行：比例 + 比例依据（不可跳过）。
2. 图片角色判断：必须使用 `image1 / image2 / image3...` 编号，逐张写清“是哪张图 + 负责什么 + 不负责什么”。
3. 修改要点 2-4 条短句。
4. 最终提示词放进代码块，方便整段复制；代码块正文必须采用三段式：
   - 第一段：`image1 / image2 / image3...` 角色说明。
   - 第二段：主改图指令，尽量一段写完。
   - 第三段：简短约束，集中写“不要...”。

用户要“纯提示词”时，只输出代码块里的提示词正文，但仍必须在正文开头保留 `image1 / image2 / image3...` 角色说明。
