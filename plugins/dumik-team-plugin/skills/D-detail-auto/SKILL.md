...
- 图片接口可用（按 `D-image-run` 的 API 缓存规则）。
...
调 `D-detail-plan`：固化/复用视觉设定（含视觉调性一句话），吃卖点清单，排详情页结构，给每个卖点的视觉证据策略 + 3-4 个创意方向。

### 2. pilot — 试跑一个卖点

选**第一个卖点**，调 `D-grid-prompt` 段一（从 prompt-pack family 选方向）写 2x2 探方向提示词，调 `D-image-run` 出 pilot 图（`count: 1`，落 `输出/确认图/`）。
...
据此回写视觉设定的校准层（交 `D-detail-plan`），再用**校准后的视觉设定**给**所有卖点**重新生成抽卡提示词（交 `D-grid-prompt`）。
...
对所有卖点的选中方向，按 `D-grid-prompt` 段二展开多行独立全幅候选（每行 `count:1`、不同 id/output_name、带 `selected_direction_id` 和 `route_id`）。交 `D-image-run` 批量并发生成，落 `输出/确认图/`。
...
对 repair 的图（和自检标 drift 的），调 `D-image-repair`：先分诊（可接受/局部修/重抽），对 blocked 的对照视觉设定产品白底图诊断偏差区域。
批量修复走 `D-image-repair/scripts/repair_batch.py`：汇总修复清单 → `crop-batch` 批量切+出修复任务 → `D-image-run` banana2 批量重绘 → `paste-batch` 一图多区域链式贴回。全程无人值守。
...
- 自己不写最终提示词、不手搓接口参数：词交 `D-grid-prompt`/`D-image-repair`，生成交 `D-image-run`。
