...
## Environment
- All live image calls must use JuAIHub only:
  - Base URL: `https://api.juaihub.cn/`
  - SDK base URL used by the CLI: `https://api.juaihub.cn/v1`
- The bundled CLI reads `JUAIHUB_API_KEY`.
- This public team plugin must not store API keys. Set `JUAIHUB_API_KEY` locally before live calls.
- Do not ask for or use `OPENAI_API_KEY` for this skill. Do not switch to the official OpenAI API unless the user explicitly asks to rewrite the skill again.
...
- Require `JUAIHUB_API_KEY` before any live API call.
