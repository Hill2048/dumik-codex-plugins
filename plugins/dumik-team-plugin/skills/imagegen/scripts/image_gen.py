#!/usr/bin/env python3
"""Generate or edit images with the JuAIHub OpenAI-compatible Image API.

Defaults to gpt-image-2 through JuAIHub and a structured prompt augmentation workflow.
"""
...
JUAIHUB_BASE_URL = "https://api.juaihub.cn/v1"
...
def _ensure_api_key(dry_run: bool) -> None:
    if _juaihub_api_key():
        print("JuAIHub API key is configured.", file=sys.stderr)
        return
    if dry_run:
...
def _validate_model(model: str) -> None:
    if model != DEFAULT_MODEL:
        _die(f"model is locked to {DEFAULT_MODEL}.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate or edit images via the JuAIHub Image API")
