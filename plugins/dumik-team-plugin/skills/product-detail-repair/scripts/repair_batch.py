...
# -*- coding: utf-8 -*-
"""
product-detail-repair 批量修复驱动。

把「一堆选中图 × 每图多个偏差区域」的修复跑成三步，无人值守：

  1) crop-batch  读修复清单 -> 切出所有区域 + 产出修复任务 JSON（给 image-batch-agent）
  2) image-batch-agent 用 banana2 + 产品白底图重绘所有切片（不在本脚本，见 SKILL）
...
def do_crop_batch(args):
    ...
    print(f"[crop-batch] 修复任务 JSON -> {args.jobs}（交 image-batch-agent，banana2，输出到 repaired-dir）")
...
def main():
    p = argparse.ArgumentParser(description="product-detail-repair 批量修复驱动")
    ...
    pp.add_argument("--repaired-dir", required=True, help="image-batch-agent 修好的切片目录")
