...
启动时先参考插件统一路由：`assets\agent-skill-routing.md`。本技能只写视频旁路的判断、项目入口、调度和检查；具体转化逻辑、故事板或生视频执行交给底层技能。

同时负责本次视频项目的前置判断和环境部署：当任务进入项目制、批量生视频、确认片、关键帧生图或多轮制作时，先按项目入口协议确认项目，再创建项目文件夹、分好素材和产物分类、检查 Dreamina / Veo / 图片生成所需工具；工具缺失时优先自动安装或初始化，不能自动解决时再停下来说明缺口。
...
- 无 `CURRENT_PROJECT.md`，且用户要项目制、批量、多轮沉淀、确认片后批量：先问项目名和项目类型。
...
- 用户只是要一段脚本、一次分镜、一次单条视频提示词，且没有项目制表达：按单次任务处理，不强行建项目。
