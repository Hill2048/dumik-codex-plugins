...
def read_existing_constant(path: Path, name: str) -> str:
    if not path.exists():
        return ""
...
def write_cache(path: Path, base_url: str, api_key: str, source: str) -> None:
    ...
    runninghub_base_url = read_existing_constant(path, "RUNNINGHUB_API_BASE_URL")
    runninghub_api_key = read_existing_constant(path, "RUNNINGHUB_API_KEY")
    payload = "\n".join(
        [
            ...
            f"API_KEY = {api_key!r}",
            f"RUNNINGHUB_API_BASE_URL = {runninghub_base_url!r}",
            f"RUNNINGHUB_API_KEY = {runninghub_api_key!r}",
            "",
        ]
    )
