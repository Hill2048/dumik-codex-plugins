...
(function () {
  ...
  function prepareRelinkTasks(missingItems, fileIndex, options) {
    options = options || {};
    var tasks = [];
    ...
    for (var j = 0; j < missingItems.length; j++) {
      ...
      var expected = expectedFileName(item);
      if (!expected) {
        if (options.deferUnresolved && options.unresolved) {
          options.unresolved.push(item);
          log("待补选，无法读取原文件名: " + item.path + " | " + item.meta.fileReference);
        } else {
          failedCount++;
      ...
      if (matches.length === 0) {
        if (options.deferUnresolved && options.unresolved) {
          options.unresolved.push(item);
          log("待补选，未找到同名文件: " + item.path + " | " + expected);
        } else {
          failedCount++;
      ...
      if (matches.length > 1 || resolved.ambiguous) {
        if (options.deferUnresolved && options.unresolved) {
          options.unresolved.push(item);
          log("待补选，候选多个: " + item.path + " | " + expected + " | 规则 " + resolved.reason + " | 数量 " + matches.length);
        } else {
          skippedCount++;
      ...
      try {
        ...
        if (options.deferUnresolved && options.unresolved) {
          options.unresolved.push(item);
          log("待补选，准备重链失败: " + item.path + " | " + expected + " | " + ePrepare);
        } else {
          failedCount++;
  ...
  function prepareProxyRelinkTasks(proxyMissingItems, fileIndex, options) {
    var tasks = [];
    for (var i = 0; i < proxyMissingItems.length; i++) {
      var item = proxyMissingItems[i];
      var prepared = prepareRelinkTasks([item], fileIndex, options);
  ...
  function buildFileIndex(folder, missingItems) {
    var fileIndex = newFileIndex();
    var targetKeys = targetKeysForMissing(missingItems);
    scanFolder(folder, fileIndex, targetKeys);
    log("已扫描目录: " + folder.fsName + " | 目录: " + fileIndex.stats.folders + " | 文件: " + fileIndex.stats.files + " | 最大深度: " + MAX_SEARCH_DEPTH);
    return fileIndex;
  }
  ...
  try {
    ...
    var ok = confirm(
      "找到丢失链接智能对象：" + (missing.length + proxyMissing.length) + " 个\n" +
      "其中代理内部：" + proxyMissing.length + " 个\n\n" +
      "先自动查主文件旁边的 links / 同目录。\n" +
      "如果没找全，会再弹出文件夹让你补选。\n\n" +
    ...
    function runPreparedTasks(historyName) {
      if (!relinkTasks.length && !proxyRelinkTasks.length) return;
      runAsOneHistory(historyName, relinkAllMissingSmartObjects);
    }

    var unresolved = missing.concat(proxyMissing);

    if (searchFolder) {
      log("自动搜索目录: " + searchFolder.fsName);
      var autoIndex = buildFileIndex(searchFolder, unresolved);
      var unresolvedAfterAuto = [];
      relinkTasks = prepareRelinkTasks(missing, autoIndex, { deferUnresolved: true, unresolved: unresolvedAfterAuto });
      proxyRelinkTasks = prepareProxyRelinkTasks(proxyMissing, autoIndex, { deferUnresolved: true, unresolved: unresolvedAfterAuto });
      runPreparedTasks("自动重新链接丢失智能对象");
      unresolved = unresolvedAfterAuto;
    }

    if (unresolved.length > 0) {
      var manualTitle = searchFolder
        ? "还有 " + unresolved.length + " 个没找回。请选择补充搜索文件夹。"
        : "选择用于查找丢失链接文件的文件夹";
      var manualFolder = Folder.selectDialog(manualTitle);
      if (manualFolder) {
        log((searchFolder ? "补充搜索目录: " : "手动搜索目录: ") + manualFolder.fsName);
        var manualIndex = buildFileIndex(manualFolder, unresolved);
        var unresolvedMain = [];
        var unresolvedProxy = [];
        for (var u = 0; u < unresolved.length; u++) {
          if (unresolved[u].proxy) unresolvedProxy.push(unresolved[u]);
          else unresolvedMain.push(unresolved[u]);
        }
        relinkTasks = prepareRelinkTasks(unresolvedMain, manualIndex);
        proxyRelinkTasks = prepareProxyRelinkTasks(unresolvedProxy, manualIndex);
        runPreparedTasks("补充重新链接丢失智能对象");
      } else {
        skippedCount += unresolved.length;
        log("用户取消补充搜索，剩余未处理: " + unresolved.length);
      }
    }
