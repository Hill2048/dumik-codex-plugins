...
- 图片生成脚本存在：`skills\D-image-run\scripts\generate_batch_images.py`。
- 视频生成脚本存在：`skills\D-video-run\scripts\generate_batch_videos.py`。
...
- 使用 Banana2 本地参考图公网 URL 前，仍需轻量确认当前 tunnel URL 可达；不可达时运行 `skills\D-image-run\scripts\start_nas_public_tunnel.ps1` 重启，并将 `nas_image_url.json` 标准化为 UTF-8 无 BOM JSON。
